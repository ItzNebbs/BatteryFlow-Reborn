#import "BFRFirstBanner.h"

@implementation BFRFirstBanner
+ (NSString *)hb_specifierPlist {
	return @"FirstBanner";
}
- (NSMutableArray *)specifiers {
	if (!_specifiers) {
		_specifiers = [self loadSpecifiersFromPlistName:@"FirstBanner" target:self];
...
- (void)testFirstBanner {
	CFNotificationCenterPostNotification(CFNotificationCenterGetDarwinNotifyCenter(), CFSTR("xyz.itznebbs.batteryflow-reborn/firstbannertest"), nil, nil, true);
}
@end