#import "BFRFourthBanner.h"

@implementation BFRFourthBanner
+ (NSString *)hb_specifierPlist {
	return @"FourthBanner";
}
- (NSMutableArray *)specifiers {
	if (!_specifiers) {
		_specifiers = [self loadSpecifiersFromPlistName:@"FourthBanner" target:self];
...
- (void)testFourthBanner {
	CFNotificationCenterPostNotification(CFNotificationCenterGetDarwinNotifyCenter(), CFSTR("xyz.itznebbs.batteryflow-reborn/fourthbannertest"), nil, nil, true);
}
@end