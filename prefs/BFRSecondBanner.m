#import "BFRSecondBanner.h"

@implementation BFRSecondBanner
+ (NSString *)hb_specifierPlist {
	return @"SecondBanner";
}
- (NSMutableArray *)specifiers {
	if (!_specifiers) {
		_specifiers = [self loadSpecifiersFromPlistName:@"SecondBanner" target:self];
...
- (void)testSecondBanner {
	CFNotificationCenterPostNotification(CFNotificationCenterGetDarwinNotifyCenter(), CFSTR("xyz.itznebbs.batteryflow-reborn/secondbannertest"), nil, nil, true);
}
@end